...
class D1AobjTwoStagePseudoLabGeneralizedRCNN(GeneralizedRCNN):
    ...
    def center_crop(self, size, images):
        center_crop = transform.CenterCrop(size)
        images = center_crop(images.tensor)
        images = ImageList.from_tensors([*images], self.backbone.size_divisibility)
        return images
    def forward(
